...
export function getChildIds<T>(items: IArrayValueMap<ITreeObject<T>>, itemId: TreeKey, collectedIds: TreeKey[] = []) {
    const item = items.valueMap[itemId];
    collectedIds.push(item.id);
    item.childs.forEach(x => {
        getChildIds(items, x.id, collectedIds);
    });
    return collectedIds;
}
...
export function array2TreeMap<T>(data: T[] = [], getId: (item: T) => TreeKey, getParentId: (item: T) => TreeKey, rootItem: T): IArrayValueMap<ITreeObject<T>> {
    const result = new Array() as unknown as IArrayValueMap<ITreeObject<T>>;
    result.valueMap = {};
    ...
    result.remove = function(id: TreeKey, childIds?: TreeKey[]) {
        ...
        const ids: TreeKey[] = childIds || getChildIds(result, id, []);
